...
import SubregionEntry from '../../components/region/SubregionEntry';
...
const SubRegionSpreadSheet = (props) => {
    let maps =[];
    const [map, setMap] = useState({});
    const[entries, setEntries] = useState([]);
    const[AddRegion]    =useMutation(mutations.ADD_REGION);

    entries = props.activeRegion.regions;
	...
	return (
        <>
        <div>
        <WButton onClick={null} className={'add-subregion-button'} >+
                        </WButton>
            ...
            {entries ? <div className=' region-spreadsheet container-primary'>             
            {
                ...
                entries.map((entry, index) => (
                    <SubregionEntry
                        ...
                        handleActive={props.setActiveRegion}
                    />
                ))
            }
...
export default SubRegionSpreadSheet;
