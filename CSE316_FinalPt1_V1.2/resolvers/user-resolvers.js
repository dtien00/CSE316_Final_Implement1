...
module.exports = {
	...
	Mutation: {
		...
		**/
		...
		updateUser: async (_, args, { res }) => {
			console.log("REACHED");
			const { email, name, password, currentEmail} = args;
			console.log("Name: " + name);
			console.log("Current email: " + currentEmail);
			const alreadyRegistered = await User.findOne({email: currentEmail});
			console.log(alreadyRegistered);
			const hashed = await bcrypt.hash(password, 10);
			const updated = await User.updateOne({email: currentEmail}, {name: name, email: email, password: hashed});
			if(updated) return User.findOne({email: email});
